      ...
      <div style={{ position: "sticky", top: 0, height: "100vh", width: "100vw", overflow: "hidden", backgroundColor: "#000" }}>
        ...
        {!loaded && (
          <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", color: "white", zIndex: 50, backgroundColor: "#111" }}>
            ...
            <div style={{ fontFamily: "var(--font-hind)", fontSize: "1.2rem", marginBottom: "0.25rem", color: "rgba(255,255,255,0.9)" }}>{t.loading.preparing}</div>
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 10, pointerEvents: "none" }}>
          ...
          {/* THE ROAD */}
          <motion.div className={`overlay-container overlay-road ${language === 'ur' || language === 'ks' || language === 'sd' ? 'rtl-align' : ''}`} style={{ position: "absolute", top: "55%", translate: "0 -50%", opacity: roadOpacity, y: roadY, textAlign: language === 'ur' || language === 'ks' || language === 'sd' ? 'right' : 'left' }}>
            <h2 className="text-fluid-hero" style={{ fontFamily: "var(--font-yatra-one)", color: "white", textShadow: "0 2px 12px rgba(0,0,0,0.5)", marginBottom: "0.75rem", whiteSpace: "pre-line" }}>
              {t.hero.roadHeading}
            </h2>
          ...
          {/* THE DISCOVERY */}
          <motion.div className={`overlay-container overlay-discovery ${language === 'ur' || language === 'ks' || language === 'sd' ? 'rtl-align' : ''}`} style={{ position: "absolute", top: "50%", translate: "0 -50%", opacity: discOpacity, y: discY, textAlign: language === 'ur' || language === 'ks' || language === 'sd' ? 'left' : 'right' }}>
            <h2 className="text-fluid-hero" style={{ fontFamily: "var(--font-yatra-one)", color: "white", textShadow: "0 2px 12px rgba(0,0,0,0.5)", marginBottom: "0.75rem", whiteSpace: "pre-line" }}>
              {t.hero.discoveryHeading}
            </h2>
            <p className="text-fluid-body" style={{ fontFamily: "var(--font-hind)", color: "white", textShadow: "0 1px 6px rgba(0,0,0,0.6)", marginTop: "1.5rem", marginLeft: language === 'ur' || language === 'ks' || language === 'sd' ? 0 : "auto", marginRight: language === 'ur' || language === 'ks' || language === 'sd' ? "auto" : 0 }}>
              {t.hero.discoveryBody}
            </p>
          </motion.div>
          ...
          {/* CONNECTED */}
          <motion.div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: language === 'ur' || language === 'ks' || language === 'sd' ? "flex-start" : "flex-end", justifyContent: "flex-end", paddingRight: "5%", paddingBottom: "5%", opacity: connectedOpacity, y: connectedY }}>
            <button 
            ...
            >
              <PhoneCall size={32} style={{ marginInlineEnd: "16px", flexShrink: 0 }} />
              <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", textAlign: "start", flex: "1 1 auto", minWidth: 0, overflowWrap: "anywhere" }}>
                <span className="connected-btn-title" style={{ fontFamily: "var(--font-yatra-one)", lineHeight: 1.2, marginBottom: "0.25rem", whiteSpace: "normal" }}>{t.cta.callTollFree}</span>
                <span className="connected-btn-sub" style={{ opacity: 0.9, unicodeBidi: "isolate", direction: "ltr" }}>1800-XXXXXXX</span>
              </div>
            </button>
          </motion.div>
        </div>
